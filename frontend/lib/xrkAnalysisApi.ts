...
export type XrkInspection = {
  ...
  parser: { library: string; version: string; license: string; status: string };
  warnings: string[];
};
...
export async function inspectXrkFile(
  ...
  try {
    response = await fetch(apiUrl("/xrk/inspect"), {
      method: "POST",
      body: form,
      signal,
    });
    ...
    throw new Error("XRK inspection service is unavailable. CSV and Demo remain available.");
  }
  if (!response.ok) {
    throw new Error(await responseError(response, `XRK inspection failed (${response.status}).`));
  }
...
export async function analyzeXrkInspection(
  ...
  const response = await fetch(apiUrl("/xrk/analyze"), {
    method: "POST",
  ...
  if (!response.ok) {
    throw new Error(await responseError(response, `XRK analysis failed (${response.status}).`));
  }
  return response.json() as Promise<XrkAnalysis>;
}

export async function deleteXrkInspection(inspectionId: string): Promise<void> {
  await fetch(apiUrl(`/xrk/inspections/${inspectionId}`), { method: "DELETE" }).catch(() => {
