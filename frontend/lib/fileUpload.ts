...
export function describeFileReadError(error: unknown): string {
  ...
  if (name === "NotReadableError" || name === "SecurityError") {
    return "系统拒绝了浏览器读取该文件。若文件位于外置磁盘或 iCloud，请先把文件完整复制到桌面或下载目录，确认可读取后重新选择。";
  }
...
/** Browser supplies the multipart boundary; detached bytes survive Safari picker cleanup. */
export function multipartFileUploadRequest(blob: Blob, filename: string, signal?: AbortSignal): RequestInit {
