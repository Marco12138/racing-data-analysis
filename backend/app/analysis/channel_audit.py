...
def spectral_summary(times: np.ndarray, values: np.ndarray, target_rate_hz: float) -> dict[str, Any]:
    ...
    if len(t) < 64 or np.any(np.diff(t) <= 0):
        return {"status": "unavailable", "reason": "insufficient_or_nonmonotonic_samples"}
    ...
    if not chunks:
        return {"status": "unavailable", "reason": "no_contiguous_window"}
        ...
        for axis in ("gyro_x", "gyro_y", "gyro_z"):
            ...
            for chunk in np.array_split(np.arange(len(grid)), 3):
                ...
                for lag in np.arange(-1, 1.001, .05):
                    ts, vals = native_series(native, selected[axis])
